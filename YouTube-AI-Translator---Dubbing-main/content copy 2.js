...
const TTS_RATE = 3;
const SUBTITLE_LEAD_S = 10;     // Hiển thị phụ đề và kích hoạt TTS sớm hơn N giây so với video audio
...
function enqueueLiveCaption(text, allowMerge = true) {
  const now = Date.now();
  ...
  if (allowMerge && liveCaptionQueue.length > 0
  ...
  liveCaptionObserver = new MutationObserver(() => {
    ...
    if (bottomLine !== liveCaptionLastLineText) {
      ...
      if (previousLine
        ...
        enqueueLiveCaption(previousLine, false);
      }
    ...
    if (topLine && topLine !== liveCaptionLastTopLine) {
      if (!completedLine || topLine !== completedLine) {
        enqueueLiveCaption(topLine, false);
      }
  ...
  currentTimeUpdateHandler = () => {
    try {
      ...
      if (displayIndex !== -1 && displayIndex !== lastDisplayedIndex) {
        ...
        // Trigger TTS ngay khi subtitle xuất hiện (sớm 10s so với video audio)
        // → Web Speech API có đủ thời gian khởi động và đọc xong
        // Trong syncSubtitles(), thay đoạn TTS (dòng 1332–1341):

        // TRƯỚC (cancel + speak ngay lập tức)
        if (isDubbingEnabled && 'speechSynthesis' in window && displayIndex !== lastSpokenIndex) {
          speechSynthesis.cancel();
          const utterance = new SpeechSynthesisUtterance(...);
          speechSynthesis.speak(utterance);
          lastSpokenIndex = displayIndex;
        }

        // SAU (queue, không cancel nếu đang nói)
        if (isDubbingEnabled && 'speechSynthesis' in window && displayIndex !== lastSpokenIndex) {
          lastSpokenIndex = displayIndex;
          const text = translatedSegments[displayIndex].translatedText;
          // Nếu TTS đang nói → enqueue, nếu rảnh → nói ngay
          if (speechSynthesis.speaking) {
            // Chỉ giữ 1 item trong queue (segment liền kề tiếp theo)
            liveCaptionQueue.length = 0;
            liveCaptionQueue.push(text);
          } else {
            speakText(text, true);
          }
        }
